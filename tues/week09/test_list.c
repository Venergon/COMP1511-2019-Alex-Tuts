...
void test_list_append(void) {
    ...
    struct node *test1 = create_list(vals1, 3);
    struct node *test2 = create_list(vals2, 3);

    struct node *full_list = list_append(test1, test2);

    assert(num_items(full_list) == 6);
    assert(full_list->data == 1);
    assert(full_list->next->data == 2);
    assert(full_list->next->next->data == 3);

    assert(full_list->next->next->next->data == 1);
    assert(full_list->next->next->next->next->data == 3);
    assert(full_list->next->next->next->next->next->data == 5);
    assert(full_list->next->next->next->next->next->next == NULL);
